import $ from 'jquery'
import { ExtensionItem, Rank, RankEnum, RankScoreEnum } from '@/types/types'

const newfilterCheckboxElement = (): JQuery<HTMLElement> => $(`
  <div>
    <div>Filter</div>
  </div>
`)
const newFilterCheckbox = (name: Rank, value: Rank, text: string | null, src: string | null): JQuery<HTMLElement> => $(`
...
`)
const rankScoreRange = (rank: Rank): ((score: number) => boolean) => {
  switch (rank){
...
export default function(){
  // filter
  const filterCheckboxElement = newfilterCheckboxElement()
